...
contract PromiseAddress {
    address _parent;
    string _promiseId;
    string _promiseUrl;

    function PromiseAddress(string promiseId, string promiseUrl) {
        _parent = msg.sender;
        _promiseId = promiseId;
        _promiseUrl = promiseUrl;
    }

    function() {
        if(msg.value==0 || bytes(_promiseId).length==0 || bytes(_promiseUrl).length==0 || _parent==0) throw;
        StackExchangeBounty c = StackExchangeBounty(parent);
        c.supportPromise.value(msg.value)(_promiseId, _promiseUrl, msg.sender);
    }
...
contract StackExchangeBounty is usingOraclize {
    ...
    function registerPromise(string , String promiseUrl) internal {
        if (promises[promiseId] != 0) { exit; }
        
        // TODO: query oraclize for promise to get politicianId and determine isOpen
        PromiseAddress newPromiseAddress = new PromiseAddress(promiseId, promiseUrl);
        
        address politiciansAddress = // TODO: lookup politicians address from the politicians Id we loko up above
        promise newPromise = promise(promiseId, promiseUrl, true, )
    
        promise(promiseId, promiseUrl, true, );
        
        promise.subContractAddress = newPromise
        promises[promiseId] = newPromise;
    }
    
    function supportPromise(promiseId, promiseUrl, address supporterAddress) {
        
    }
    
    // TODO function for registering politicianId and the receiving address
